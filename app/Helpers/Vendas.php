...
class Vendas {
    public static function vendas($dataI,$dataF,$cliente,$vendedor,$nf) {
            ...
            FROM(
                ...
                /*******************ROMANEIO*****************************************************************************/
                    ...
                    Select
                        ...
                        ,PARTICIPANTE.PART_NOME	                    AS VENDEDOR
                    ...
                    LEFT JOIN PARTICIPANTE	ON PARTICIPANTE.PART_REPRESENTANTE_CODIGO = DAV.PART_REPRESENTANTE_CODIGO
    ...
    public static function vendasDetalhe($dataI,$dataF) {
        $sql = "
            SELECT
                'Sistema' 									AS ORIGEM
                ,'VENDA' 									AS TIPO
                ,'NOTA FISCAL' 								AS TIPO_RECEITA
                ,'DECORBRAS' 			   					AS EMPRESA
                ,NF.NF_NUMERO		   		   				AS DOCUMENTO
                ,NF.NS_SERIE			   					AS SERIE_FAT
                ,NF.NF_DT_EMISSAO 		   	 				AS DATA_COMP
                ,DAV.DAV_NUMERO_AUXILIAR					AS NUM_PED
                ,DAV.DAV_DATA		   						AS DATA_PED
                ...
                ,ITENS.NI_VL_TOTAL 				 			AS FAT_BRUTO
                ,CASE
                    WHEN (DAVI_VL_UNIT-NI_VL_UNITARIO) >0 THEN (DAVI_VL_UNIT-NI_VL_UNITARIO)  * NI_QTDE
                    ELSE 0
                END											AS FAT_BRUTO_2
                ,ITENS.NI_VL_DESCONTO			 			AS DESCONTO
                , '' 										AS ACRESCIMO
                ,ITENS.NI_VL_TOTAL-ITENS.NI_VL_DESCONTO  	AS FAT_LIQUIDO
                ,ITENS.NI_VL_IPI				 			AS PIS
                ,ITENS.NI_VL_COFINS							AS COFINS
                ,ITENS.NI_VL_ICMS               			AS ICMS
                ,NF.NF_VL_CSLL								AS CSLL
                ,''											AS IRPJ
                ...
                ,'' 										AS PERC_COMISSAO
                ,ITENS.NI_COMISSAO              			AS COMISSAO
                ,NF.NF_VL_FRETE								AS FRETE_TERCEIROS
            ...
            LEFT JOIN PAIS ON PAIS.PAIS_CODIGO = NF.PAIS_CODIGO
            LEFT JOIN ENTIDADES AS REP ON REP.ENT_CODIGO = NF.PART_REPRESENTANTE_CODIGO
                                    AND REP.ENT_TIPO = 3
            ...
            AND CFOP_GRUPOS.CFGR_CODIGO = 1
            AND NF_SITUACAO IN(1)
            AND NF.NF_DT_EMISSAO BETWEEN '$dataI' AND '$dataF'
            ...
            Select
                ...
                ,MUNICIPIO.UF_CODIGO			        AS ESTADO
                ,''							  			AS PAIS
                ,DAV.PART_CODIGO						AS COD_VENDEDOR
                ,PARTICIPANTE.PART_NOME	                AS VENDEDOR
                ,CONDICOES_PAGAMENTO.CPA_DESCRICAO		AS FORMA_DE_PAGTO
                ,DAV_ITENS.PRD_CODIGO					AS COD_PRODUTO
                ,DAV_ITENS.DAVI_DESCRICAO  				AS PRODUTO
                ...
                ,DAVI_VL_UNIT
                ,0								 		AS FAT_BRUTO
                ,DAV_ITENS.DAVI_VL_TOTAL_BRUTO		  	AS FAT_BRUTO_2
                ,DAV_ITENS.DAVI_VL_DESCONTO		 	   	AS DESCONTO
                ,DAV_ITENS.DAVI_VL_ACRESCIMO			AS ACRESCIMO
                ,DAV_ITENS.DAVI_VL_TOTAL                AS FAT_LIQUIDO
                ...
                ,DAV_ITENS.DAVI_COMISSAO_PERC			AS PERC_COMISSAO
                ,DAV_ITENS.DAVI_VALOR_COMISSAO          AS COMISSAO
                ,DAV_ITENS.DAVI_FRETE_VALOR_TOTAL		AS FRETE_TERCEIROS

            FROM DAV
            LEFT JOIN DAV_ITENS		ON DAV_ITENS.DAV_NUMERO = DAV.DAV_NUMERO
            LEFT JOIN MUNICIPIO		ON MUNICIPIO.MUN_CODIGO = DAV.MUN_CODIGO
            LEFT JOIN PARTICIPANTE	ON PARTICIPANTE.PART_CODIGO = DAV.PART_REPRESENTANTE_CODIGO
            LEFT JOIN CONDICOES_PAGAMENTO ON CONDICOES_PAGAMENTO.CPA_CODIGO = DAV.CON_TIPO_PAGAMENTO

            WHERE 1=1
            AND DSP_CODIGO = 1
            AND DAV_DATA_EMISSAO BETWEEN '$dataI' AND '$dataF'
        ";
        // dd($sql);
        $vendas = DB::connection(env('APP_NAME'))->select($sql);
        return $vendas;
